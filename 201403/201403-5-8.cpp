	...
	for (int q = 1; q <= n; q++) {
		for (int i = 0; i <= t && i <= q * 10; i++) {
			for (int j = i; j <= t && j <= q * 10; j++) {
				int &v = dp[q & 1][i][j];
				v = 0x7f7f7f7f; // assign an initial value to the reference
				if (i >= a[q]) {
					v = min(v, dp[(q - 1) & 1][i - a[q]][j]);
				}
				if (j >= a[q]) {
					v = min(v, dp[(q - 1) & 1][min(i, j - a[q])][max(i, j - a[q])]);
				}
				if (i >= c[q]) {
					v = min(v, dp[(q - 1) & 1][i - c[q]][j] + c[q]);
				}
				if (j >= c[q]) {
					v = min(v, dp[(q - 1) & 1][min(i, j - c[q])][max(i, j - c[q])] + c[q]);
				}
				if (i >= d[q] && j >= d[q]) {
