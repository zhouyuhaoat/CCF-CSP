...
void bfs(int s) {
	...
	while (!q.empty()) {
		...
		q.pop();
		// int cnt = u.second; // not here, since cnt is not constant
		for (auto v : g[u.first]) {
			int cnt = u.second; // of current path
			if (v >= n) {
				++cnt; // merge new router and existed router, to unified relaxing
			}
			if (cnt <= k && d[v][cnt] > d[u.first][u.second] + 1) {
