/*
 * 	author: 	zhouyuhao
 * 	created: 	2024-05-31 15:22:15
 * 	modified: 	2024-06-01 16:47:37
...
int minf[maxv], top;
// minf: father/parent/ancestor vertex with minimum price from root
// top - 1: index to the vertex
	...
	for (auto qq : qry[v]) {
		...
		if (pri[u][16] <= p[minf[top - 1]]) {
			...
			while (l < r) { // binary search
				// r: last vertex with price <= pri[u][16]
				// r: first vertex with price > pri[u][16] - 1
				// l: first vertex with price > pri[u][16]
				int mid = (l + r) / 2;
				if (p[minf[mid]] <= pri[u][16]) {
					r = mid;
				} else {
					l = mid + 1;
				}
...
void calc(int u) {
	...
	down[u] = 0, minf[0] = u, top = 1; // index: top - 1
	for (auto e : g[u]) {
		auto [v, w] = e;
