/*
 * 	author: 	zhouyuhao
 * 	created: 	2024-05-28 17:37:25
 * 	modified: 	2024-06-01 17:10:09
 * 	item: 		Certified Software Professional
 * 	site: 		914, Harbin
 */
// https://www.acwing.com/solution/content/153871/
#include <algorithm>
#include <cstring>
#include <iostream>
#include <vector>
...
void dfs2(int u, int fa) {
	...
	if (minp[top - 1] < pr[u]) {
		...
		minp[top] = pr[u], minu[top] = u;
	}
	++top;

	for (int i = 0; i < (int)qt[u].size(); i++) {
		...
		if (cm[a] <= minp[top - 1]) {
			...
			// int r = upper_bound(minp, minp + top, cm[a], greater<int>()) - minp;
			// int r = lower_bound(minp, minp + top, cm[a], greater<int>()) - minp;
			int ver = minu[r];
